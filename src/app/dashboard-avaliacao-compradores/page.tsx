...
// NOTA: Certifique-se de que o caminho para o seu cliente Supabase está correto.
import { createClient } from '@/utils/supabase/client'; 
import type { Launch } from "@/lib/types";
import { Users, ArrowRightLeft, TrendingUp, TrendingDown, ChevronsUpDown, BarChart2, ArrowDown, ArrowUp } from "lucide-react";

// --- Tipos de Dados (v6 - com classe) ---
type AnswerComparison = {
    checkin_text: string | null;
    checkin_points: number;
    compra_text: string | null;
    compra_points: number;
    classe: string; // NOVO CAMPO
};
...
type ComparisonData = {
    ...
    score_diff: number;
    answers_comparison: null | {
        [questionText: string]: AnswerComparison;
    };
};

type SortConfig = {
    key: keyof Omit<ComparisonData, 'lead_id' | 'answers_comparison'>;
...
// --- COMPONENTE DE DETALHAMENTO APRIMORADO (v6 - com classe) ---
    ...
    if (!lead.answers_comparison || Object.keys(lead.answers_comparison).length === 0) {
        return (
            <div className="p-6 bg-slate-50 border-t-2 border-emerald-200">
                <p className="text-slate-600 text-center italic">Este comprador não possui respostas detalhadas para análise.</p>
            </div>
        );
    }

    // Ordena as perguntas para mostrar 'Score' primeiro
    const sortedAnswers = Object.entries(lead.answers_comparison).sort(([, a], [, b]) => {
        if (a.classe === 'Score' && b.classe !== 'Score') return -1;
        if (a.classe !== 'Score' && b.classe === 'Score') return 1;
        return 0;
    });
                    ...
                    return (
                        <div key={question} className="border-b border-slate-200 pb-4 last:border-b-0">
                            <div className="flex items-center gap-3 mb-3">
                                ...
                                <span className={`uppercase px-2 py-0.5 text-xs font-bold rounded-full ${data.classe === 'Score' ? 'bg-emerald-100 text-emerald-800' : 'bg-sky-100 text-sky-800'}`}>
                                    {data.classe}
                                </span>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                ...
                                {hasCheckinAnswer ? (
                                    <div className="p-3 bg-blue-50 rounded-md border border-blue-200">
                                        <p className="text-xs font-semibold text-blue-700 mb-1">RESPOSTA NO CHECK-IN</p>
                                        <p className="text-slate-800">"{data.checkin_text}"</p>
                                        <p className="mt-2 text-xs text-blue-600">Pontos: <strong>{data.checkin_points}</strong></p>
                                    </div>
                                ) : (
                                    <div className="p-3 bg-gray-100 rounded-md border border-gray-200 flex items-center justify-center">
                                        <p className="text-sm text-gray-500 italic">Não respondido no check-in</p>
                                    </div>
                                )}
                                ...
                                {hasPurchaseAnswer ? (
                                     areAnswersSame ? (
                                        <div className="p-3 bg-slate-100 rounded-md border border-slate-200">
                                            <p className="text-slate-800 font-medium">"{data.checkin_text}"</p>
                                            <p className="text-xs text-slate-500 mt-1 italic">A resposta não mudou.</p>
                                            <div className={`mt-2 text-xs text-slate-500 flex items-center gap-4 transition-colors duration-300 ${!arePointsSame ? 'p-2 bg-yellow-100 rounded-md' : ''}`}>
                                                <span>Pontos Compra: <strong className={arePointsSame ? 'text-slate-800' : 'text-emerald-600 font-bold'}>{data.compra_points}</strong></span>
                                                {!arePointsSame && <span className="font-bold text-yellow-700">(Pontuação Mudou!)</span>}
                                            </div>
                                        </div>
                                     ) : (
                                        <div className="p-3 bg-emerald-50 rounded-md border border-emerald-200">
                                            <p className="text-xs font-semibold text-emerald-700 mb-1">RESPOSTA NA COMPRA</p>
                                            <p className="text-slate-800">"{data.compra_text}"</p>
                                            <p className="mt-2 text-xs text-emerald-600">Pontos: <strong>{data.compra_points}</strong></p>
                                        </div>
                                    )
                                ) : (
                                    <div className="p-3 bg-gray-100 rounded-md border border-gray-200 flex items-center justify-center">
                                        <p className="text-sm text-gray-500 italic">Não respondido na pesquisa de comprador</p>
                                    </div>
                                )}
                            </div>
                        </div>
...
export default function AvaliacaoCompradoresPage() {
    ...
    const fetchData = useCallback(async (launchId: string) => {
        ...
        try {
            const { data, error } = await supabase.rpc('get_buyer_comparison', { p_launch_id: rpcLaunchId });
    ...
    const kpis = useMemo(() => {
        if (!comparisonData || comparisonData.length === 0) {
            return { total: 0, mediaVariacao: 0, aumentaram: 0, diminuiu: 0 };
        }
        ...
        <div className="p-4 sm:p-6 lg:p-8 bg-slate-100 min-h-screen">
            ...
            <main className="bg-white p-6 rounded-lg shadow-md">
                ...
                {!loading && !error && comparisonData.length === 0 && (
                    <div className="text-center py-10"><p className="text-gray-600">Nenhum comprador foi encontrado para análise neste lançamento.</p></div>
                )}
                {!loading && !error && comparisonData.length > 0 && (
                                ...
                                {sortedData.map((lead) => (
                                    <Fragment key={lead.lead_id}>
                                        <tr className="border-b border-slate-200 hover:bg-slate-50">
                                            ...
                                            <td className={`px-4 py-3 text-center font-bold ${lead.score_diff > 0 ? 'text-green-600' : lead.score_diff < 0 ? 'text-red-600' : 'text-slate-500'}`}>
                                                {lead.score_diff > 0 ? `+${lead.score_diff}` : lead.score_diff}
                                            </td>
                                            <td className="px-4 py-3 text-center">
                                                <button 
                                                    ...
                                                    title="Ver/Ocultar detalhes das respostas"
                                                >
                                        ...
                                        {expandedLeadId === lead.lead_id && (
                                            <tr>
                                                <td colSpan={5}>
                                                    <ComparisonDetailCard lead={lead} />
                                                </td>
                                            </tr>
                                        )}
                                    </Fragment>
