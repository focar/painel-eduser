...
import { ChevronsUpDown, ArrowUp, ArrowDown, ArrowRight, XCircle } from 'lucide-react';
...
// ====================================================================================
// PONTO 1: ONDE A ORDEM DOS KPIS É DEFINIDA
// Esta constante 'profileOrder' define a ordem exata em que os perfis devem aparecer.
// A ordem aqui é "Quente" para "Frio", exatamente como você pediu.
// Qualquer componente que renderize os KPIs ou ordene a tabela usará esta constante como referência.
// ====================================================================================
const profileOrder = ['Quente', 'Quente-Morno', 'Morno', 'Morno-Frio', 'Frio'];

// Esta constante define os estilos (cores, texto) para cada KPI, baseada no seu outro dashboard.
...
// ====================================================================================
// PONTO 2: ONDE OS KPIS SÃO DEFINIDOS E VIRAM BOTÕES
// Este é o componente 'ProfileKpiCard'. Ele é responsável por desenhar cada KPI individual
// com o estilo que você pediu.
// Note que ele é um elemento <button>.
// A propriedade 'onClick' é passada para ele, o que faz com que ele funcione como um botão.
// Quando clicado, ele vai chamar a função que atualiza o filtro da tabela.
// ====================================================================================
...
export default function JornadaCompradorPage() {
    ...
    const kpiData = useMemo(() => {
        ...
        const totalBuyers = (journeyData.aggregated_data || []).reduce((sum, item) => sum + item.buyer_count, 0);
        
        // Esta linha usa a constante 'profileOrder' para garantir que os dados dos KPIs
        // são criados na ordem "Quente" para "Frio".
        return profileOrder.map(profile => {
            const count = dataMap.get(profile) || 0;
            const percentage = totalBuyers > 0 ? ((count / totalBuyers) * 100).toFixed(1) : "0.0";
                ...
                <div className="space-y-8">
                    <div className="bg-white p-6 rounded-lg shadow-md">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                            {
                                // ====================================================================================
                                // PONTO 3: ONDE OS KPIS SÃO RENDERIZADOS E A LÓGICA DO BOTÃO É APLICADA
                                // O método '.map()' itera sobre 'kpiData', que já está na ordem correta ("Quente" para "Frio").
                                // Para cada perfil, ele renderiza um componente 'ProfileKpiCard'.
                                // A propriedade 'onClick' é definida aqui. Quando o botão é clicado, ele executa
                                // a função 'setActiveProfileFilter(profile)', que atualiza o estado e filtra a tabela.
                                // ====================================================================================
                            ...
                            <button
                                ...
                                className="p-4 rounded-lg shadow-md text-center transition-all duration-200 w-full bg-slate-200 hover:bg-slate-300 text-slate-700 disabled:opacity-50 disabled:cursor-not-allowed flex flex-col justify-center items-center"
                            >
                                <XCircle size={20} />
                                <span className="text-sm font-bold mt-2">Limpar Filtro</span>
                            </button>
                        </div>
                    </div>
