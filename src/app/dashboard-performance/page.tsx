...
const KpiCard = ({ title, value, highlight = false }: { title: string, value: string | number, highlight?: boolean }) => {
    ...
    return (
        <div className={`${cardClasses} p-4 rounded-lg shadow-md border text-center md:text-left`}>
...
export default function PerformanceControlePage() {
    ...
    const loadDashboardData = useCallback(async () => {
        ...
        try {
            ...
            const { data, error } = await supabase.rpc('get_full_performance_dashboard', {
    ...
    }, [selectedLaunch, period, customStartDate, customEndDate, customStartTime, customEndTime, groupBy, supabase]);
                ...
                <div className="space-y-6">
                            ...
                            <table className="min-w-full">
                                ...
                                <tbody className="bg-white">
                                    {data.tableData.map((row, index) => {
                                        ...
                                        return (
                                            <tr key={row.canal + index} className="flex flex-col md:table-row mb-4 p-4 border rounded-lg shadow-sm md:p-0 md:border-b md:border-slate-200 md:shadow-none md:rounded-none">
                                                <td className="px-2 py-1 md:px-6 md:py-4 max-w-full truncate md:max-w-xs font-medium text-slate-800" title={row.canal}><span className="md:hidden font-normal text-slate-500">Canal: </span>{row.canal}</td>
                                                <td className="px-2 py-1 md:px-6 md:py-4 whitespace-nowrap text-sm text-slate-500"><span className="md:hidden font-normal">Inscrições: </span>{row.inscricoes}</td>
                                                <td className="px-2 py-1 md:px-6 md:py-4 whitespace-nowrap text-sm text-slate-500"><span className="md:hidden font-normal">Check-ins: </span>{row.check_ins}</td>
                                                <td className="px-2 py-1 md:px-6 md:py-4 whitespace-nowrap text-sm text-slate-500 font-semibold"><span className="md:hidden font-normal">Taxa Conv.: </span>{conversionRate.toFixed(1)}%</td>
                                            </tr>
                                        );
                                    })}
                                </tbody>
                    ...
                    <div className="w-full">
                       <ChartCard title={`Inscrições vs. Check-ins por ${groupBy === 'content' ? 'Conteúdo' : 'Campanha'}`}>
                            <BarChart data={data.tableData} margin={{ top: 5, right: 20, left: 0, bottom: 80 }}>
                                <CartesianGrid strokeDasharray="3 3" />
                                <XAxis dataKey="canal" angle={-45} textAnchor="end" height={120} interval={0} tick={{ fontSize: 10 }} />
                                <YAxis />
                                <Tooltip />
