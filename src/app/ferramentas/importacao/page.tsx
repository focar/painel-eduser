...
export default function ImportacaoPage() {
    ...
    // --- FUNÇÃO ATUALIZADA PARA O BOTÃO DE LOTE ---
    // Esta versão utiliza a API Route '/api/process-csv-lote', que é mais segura e eficiente.
    const handleBulkProfileImport = async () => {
        if (!profileFile || !selectedLaunchForProfile) {
            showAlertModal("Atenção", "Por favor, selecione um lançamento e um ficheiro CSV na seção de Perfil.");
            return;
        }

        setIsProcessing(true);
        addLog(`[INFO] Iniciando importação em LOTE via API Route...`);

        // FormData é a forma moderna e eficiente de enviar ficheiros.
        const formData = new FormData();
        formData.append('file', profileFile);
        formData.append('launch_id', selectedLaunchForProfile);

        try {
            // Chama a nossa nova rota de API intermediária.
            const response = await fetch('/api/process-csv-lote', {
                method: 'POST',
                body: formData,
            });
            ...
            if (!response.ok) {
                // Se a resposta da API não for 2xx, lança um erro com a mensagem do backend.
                throw new Error(result.message || 'Ocorreu um erro no servidor durante o processamento.');
            }
            
            // A API Route repassa a mensagem da Edge Function.
            addLog(`[SUCESSO LOTE] ${result.message}`);
            if (result.debug_log && Array.isArray(result.debug_log)) {
                result.debug_log.forEach((logLine: string) => addLog(`[BACKEND] ${logLine}`));
            }
            showAlertModal("Importação em Lote Concluída", result.message);

        } catch (error: any) {
            addLog(`[ERRO FATAL LOTE] ${error.message}`);
            showAlertModal("Erro na Importação em Lote", `Ocorreu um erro. Mensagem: ${error.message}`);
        } finally {
            setIsProcessing(false);
            if (profileFileInputRef.current) profileFileInputRef.current.value = '';
            setProfileFile(null);
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-8 p-6 md:p-10">
            ...
            <div className="bg-white p-6 rounded-lg shadow-lg space-y-6 border-t-4 border-teal-500">
                ...
                <div className="flex flex-col sm:flex-row items-center justify-end gap-4 pt-4">
                    <button 
                        onClick={handleBulkProfileImport} 
                        disabled={isProcessing || isCalculatingMql || isCalculatingScore || !selectedLaunchForProfile || !profileFile} 
                        className="w-full sm:w-auto inline-flex items-center justify-center bg-indigo-600 text-white font-bold py-3 px-8 rounded-lg hover:bg-indigo-700 disabled:opacity-50 text-base">
                        {isProcessing ? <FaSpinner className="animate-spin mr-2" /> : <FaUpload className="mr-2" />}
                        {isProcessing ? 'A Processar Lote...' : 'Importar Grande Lote'}
                    </button>
                    <button
                    ...
                    <button onClick={handleProfileSurveyImport} disabled={isProcessing || isCalculatingMql || isCalculatingScore || !selectedLaunchForProfile || !profileFile} className="w-full sm:w-auto inline-flex items-center justify-center bg-teal-600 text-white font-bold py-3 px-8 rounded-lg hover:bg-teal-700 disabled:opacity-50 text-base">
                        ...
                        {isProcessing ? 'A Processar...' : 'Importar Respostas (Padrão)'}
                    </button>
                </div>
            </div>
